...
class Triangle(object):
    def __init__(self, uri, res, chrom, start, end, moc=None, title=None, ylabel=None, correct='weight', figsize=(7, 3.5)):
        self.res = res
        self.moc = moc
        ...
        # M = np.genfromtxt(uri, delimiter=' ', filling_values=np.nan)
        # M[np.isnan(M)] = 0
        M=np.loadtxt(uri)
    ...
    def matrix_plot(self, colormap='YlOrRd', vmin=None, vmax=None, cbr_fontsize=9,
        ...
        if vmax is None:
            vmax = np.percentile(M[M.nonzero()], 95) if M.max() > 0 else 0
        if vmin is None:
            vmin = M.min()
        ...
        if not remove_label:
            ...
            _xlabel=f"chr{self.chrom}"
            if self.moc != None:
                _xlabel = f"{_xlabel}, MoC: {self.moc:.2f}"
            chrom_ax.set_xlabel(_xlabel, fontsize=18)
